...
CALIBRATION_LABEL_MAPPING = {'Bead Diameter (um)': "beadDiameter", 'Viscosity (Pa*s)': 'viscosity',
                             ...
                             'Force Response (pN/V)': "alpha", 'Distance Response (um/V)': "distanceResponse",          #alpha=forceResponse beta=distanceResponse
...
def extract_calibration_parameters(calibrationFilePath):
    ...
    """
    with nptdms.TdmsFile.open(calibrationFilePath) as tdms_file:
        tdmsMetaData1x = tdms_file.read_metadata(calibrationFilePath)["Power Spectrum Data"][
            "Fitted Power Spectrum Channel 0 (V^2/Hz)"].properties
        tdmsMetaData1y = tdms_file.read_metadata(calibrationFilePath)["Power Spectrum Data"][
            "Fitted Power Spectrum Channel 1 (V^2/Hz)"].properties
        tdmsMetaData2x = tdms_file.read_metadata(calibrationFilePath)["Power Spectrum Data"][
            "Fitted Power Spectrum Channel 2 (V^2/Hz)"].properties
        tdmsMetaData2y = tdms_file.read_metadata(calibrationFilePath)["Power Spectrum Data"][
            "Fitted Power Spectrum Channel 3 (V^2/Hz)"].properties
        CALIBRATION_LABEL_MAPPING = {'Bead Diameter (um)': "beadDiameter", 'Viscosity (Pa*s)': 'viscosity',
                                     'Temperature (C)': 'temperature', 'Lower Bound (Hz)': 'Lower Bound (Hz)',
                                     'Upper Bound (Hz)': 'Upper Bound (Hz)',
                                     'Exclusion Ranges (Hz)': 'Exclusion Ranges (Hz)',
                                     'Corner Frequency (Hz)': "cornerFrequency",
                                     'Force Response (pN/V)': "alpha", 'Distance Response (um/V)': "distanceResponse",
                                     # alpha=forceResponse beta=distanceResponse
                                     'Trap Stiffness (pN/m)': "stiffness", 'RMSE': "RMSE", 'Applied': "Applied"}
        calibrationFit = pd.DataFrame({key: 4 * [0] for key in CALIBRATION_LABEL_MAPPING.values()},
                                      index=["PSD1x", "PSD1y", "PSD2x", "PSD2y"])
        for key in CALIBRATION_LABEL_MAPPING:
            calibrationFit.loc["PSD1x", CALIBRATION_LABEL_MAPPING[key]] = tdmsMetaData1x[key]
            calibrationFit.loc["PSD1y", CALIBRATION_LABEL_MAPPING[key]] = tdmsMetaData1y[key]
            calibrationFit.loc["PSD2x", CALIBRATION_LABEL_MAPPING[key]] = tdmsMetaData2x[key]
            calibrationFit.loc["PSD2y", CALIBRATION_LABEL_MAPPING[key]] = tdmsMetaData2y[key]
...
def process_lumicks_data(data_input, thermalCalibration, foldometerCalFit=None, beadTracking=False):
    data = deepcopy(data_input)

    if foldometerCalFit is not None:
        # print("Using Python calibration")
        correctionBeta = foldometerCalFit.loc[:, "beta"] / thermalCalibration.loc[:, "beta"]

        data["PSD1xDisplacement"] = (data["PSD1ForceX"] / foldometerCalFit.loc["PSD1x", "stiffness"]) * \
                                    correctionBeta.loc["PSD1x"]
        data["PSD2xDisplacement"] = (data["PSD2ForceX"] / foldometerCalFit.loc["PSD2x", "stiffness"]) * \
                                     correctionBeta.loc["PSD2x"]
        data["PSD1ForceX"] = data["PSD1xDisplacement"] * foldometerCalFit.loc["PSD1x", "stiffness"]
        data["PSD2ForceX"] = data["PSD2xDisplacement"] * foldometerCalFit.loc["PSD2x", "stiffness"]
        data["forceX"] = (data["PSD2ForceX"] - data["PSD1ForceX"]) / 2
        calibrationFit = foldometerCalFit

        if beadTracking:
            data["surfaceSepX"] = data["trapSepX"] - foldometerCalFit.loc["PSD1x", "beadDiameter"] / 2 \
                              - foldometerCalFit.loc["PSD2x", "beadDiameter"] / 2
        else:
            data["surfaceSepX"] = data["trapSepX"] - foldometerCalFit.loc["PSD1x", "beadDiameter"] / 2 \
                              - foldometerCalFit.loc["PSD2x", "beadDiameter"] / 2 \
                              - data["PSD2ForceX"] / foldometerCalFit.loc["PSD2x", "stiffness"] \
                              + data["PSD1ForceX"] / foldometerCalFit.loc["PSD1x", "stiffness"]

    else:
        data["PSD1xDisplacement"] = data["PSD1ForceX"] / thermalCalibration.loc["PSD1x", "stiffness"]
        data["PSD2xDisplacement"] = data["PSD2ForceX"] / thermalCalibration.loc["PSD2x", "stiffness"]

        if beadTracking:
            data["surfaceSepX"] = data["trapSepX"] - thermalCalibration.loc["PSD1x", "beadDiameter"] / 2 \
                                  - thermalCalibration.loc["PSD2x", "beadDiameter"] / 2
        else:
            data["surfaceSepX"] = data["trapSepX"] - thermalCalibration.loc["PSD1x", "beadDiameter"] / 2 \
                                  - thermalCalibration.loc["PSD2x", "beadDiameter"] / 2 \
                                  - data["PSD2ForceX"] / thermalCalibration.loc["PSD1x", "stiffness"] \
                                  + data["PSD1ForceX"] / thermalCalibration.loc["PSD1x", "stiffness"]
