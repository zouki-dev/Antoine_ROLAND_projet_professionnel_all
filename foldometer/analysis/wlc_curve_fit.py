...
def calculate_extension_offset(model, data, axis="X", forceChannel="force", distanceChannel="surfaceSep", **kwargs):
    ...
    initialFoldedFit = model.fit(data[distanceChannel + axis], params=parameters, force=data[forceChannel + axis],
                                 weights=data[forceChannel + axis])
...
def wlc_series_accurate(force, contourLengthDNA=CONTOUR_LENGTH_DNA, persistenceLengthDNA=PERSISTENCE_LENGTH_DNA,
    ...
    extensionWlc = contourLengthDNA * (4 / 3 * (1 - 1 / np.sqrt(A + 1)) - 10 * B / (np.sqrt(A) * (B -1) ** 2) +A ** 1.62 / (3.55 + 3.8 * A ** 2.2) + force / stretchModulusDNA) + contourLengthProtein * (1 - 0.5 * np.sqrt(kbT / (force * persistenceLengthProtein))) + extensionOffset
    return extensionWlc
