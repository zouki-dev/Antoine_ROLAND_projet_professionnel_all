...
def assign_regions(dataNoRegion, axis="X", minRegionLength=10, verbose=False, correctRegion=True, **kwargs):
    """
    ...
    regionData = assign_rough_regions(dataNoRegion, axis, **kwargs)

    if correctRegion:
        data = label_regions(regionData, minRegionLength, verbose)
        return data
    else:
        regionData["pullingCycle"] = 0
        return regionData
