...
def load_data(folderProject, dataName):
    data = pd.read_csv(folderProject + "/" + dataName + ".csv")
    data.index = pd.to_datetime(data["time"], unit='s')
    data["bp"] = data["proteinLc"] / 0.35
    data = data.dropna()
    return data
...
data["average"] = savgol_filter(data["proteinLc"], window_length=51, polyorder=1)
...
plt.subplot(212)
plt.plot(data["time"], data["proteinLc"], color="r")
plt.xlabel("Time (s)")
...
if st.sidebar.button("Save"):
    ...
    if not os.path.exists(folderExtrusionDetection):
        os.mkdir(folderExtrusionDetection)
    with open(folderExtrusionDetection + "/" + fileName + '.csv', "w", newline="") as fp:
        wr = csv.writer(fp)
...
#-------------------------------------------------- plots --------------------------------------------------------------
mode = st.selectbox("Select mode:", ["All the extrusions", "All the data zoomed"])
if mode == "All the data zoomed":
    ...
    for timeSlice in timeSlices:
        ...
        plt.subplot(212)
        plt.plot(data["time"].loc[maskSlice], data["proteinLc"].loc[maskSlice], color="r")
        plt.plot(data["time"].loc[maskSlice*maskExtrusion], data["proteinLc"].loc[maskSlice*maskExtrusion], color="b")
        plt.xlabel("Time (s)")
        plt.ylabel("Contour length (nm)")
        plt.ylim(minContourLength, maxContourLength)
        st.pyplot()
...
if mode == "All the extrusions":
    for timeInterval in selectedIntervals:
        ...
        plt.subplot(212)
        plt.plot(data["time"].loc[maskTimeWindow], data["proteinLc"].loc[maskTimeWindow], color="r")
        plt.plot(data["time"].loc[maskTimeWindow*maskExtrusion], data["proteinLc"].loc[maskTimeWindow*maskExtrusion], color="b")
        plt.plot(data["time"].loc[maskSingleExtrusion], data["proteinLc"].loc[maskSingleExtrusion], color="g")
        plt.xlabel("Time (s)")
        plt.ylabel("Contour length (nm)")
        plt.ylim(minContourLength, maxContourLength)
        st.pyplot()
