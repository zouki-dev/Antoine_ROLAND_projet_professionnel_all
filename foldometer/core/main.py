...
class Folding(object):
    ...
    def __init__(self, filePath, protein="unknown", condition={}, setup="DT", fluorescenceFilePath=None, loadFromDict=False):
        if loadFromDict:
            self.load_from_pickle_dict(filePath)
            return
        ...
        #========Lumicks C-Trap data format========
            ...
            self.foldometerCalFit["diffusionTheory"] = ((constants.Boltzmann * (self.foldometerCalFit["temperature"] + 273.15))\
    ...
    @classmethod
    def load_from_multi_file(cls, filePath):
        obj = cls.__new__(cls)  # Does not call __init__
        super(Folding, obj).__init__()  # Don't forget to call any polymorphic base class initializers

        obj.setup = pickle.load(open(filePath+"/setup.pickle", 'rb'))
        obj.protein = pickle.load(open(filePath+"/protein.pickle", 'rb'))
        obj.filePath = pickle.load(open(filePath+"/filePath.pickle", 'rb'))
        obj.paramsWLC = pickle.load(open(filePath+"/paramsWLC.pickle", 'rb'))
        obj.allRawData = pickle.load(open(filePath+"/allRawData.pickle", 'rb'))
        obj.rawData = pickle.load(open(filePath+"/rawData.pickle", 'rb'))
        obj.allData = pickle.load(open(filePath+"/allData.pickle", 'rb'))
        obj.data = pickle.load(open(filePath+"/data.pickle", 'rb'))
        obj.metadata = pickle.load(open(filePath+"/metadata.pickle", 'rb'))
        obj.pythonCalFit = pickle.load(open(filePath+"/pythonCalFit.pickle", 'rb'))
        obj.foldometerCalFit = pickle.load(open(filePath+"/foldometerCalFit.pickle", 'rb'))
        obj.thermalCalibration = pickle.load(open(filePath+"/thermalCalibration.pickle", 'rb'))
        obj.timeInterval = pickle.load(open(filePath+"/timeInterval.pickle", 'rb'))
        return obj
    ...
    def copy(cls, m):
        ...
        obj.setup = deepcopy(m.setup)
        obj.protein = deepcopy(m.protein)
        obj.filePath = deepcopy(m.filePath)
        obj.paramsWLC = deepcopy(m.paramsWLC)
        obj.allRawData = deepcopy(m.allRawData)
        obj.rawData = deepcopy(m.rawData)
        obj.allData = deepcopy(m.allData)
        obj.data = deepcopy(m.data)
        obj.metadata = deepcopy(m.metadata)
        obj.pythonCalFit = deepcopy(m.pythonCalFit)
        obj.foldometerCalFit = deepcopy(m.foldometerCalFit)
        obj.thermalCalibration = deepcopy(m.thermalCalibration)
        obj.timeInterval = deepcopy(m.timeInterval)
        return obj
    ...
    def from_dict(cls, mDict):
        ...
        obj.setup = deepcopy(mDict["setup"])
        obj.protein = deepcopy(mDict["protein"])
        obj.filePath = deepcopy(mDict["filePath"])
        obj.paramsWLC = deepcopy(mDict["paramsWLC"])
        obj.allRawData = deepcopy(mDict["allRawData"])
        obj.rawData = deepcopy(mDict["rawData"])
        obj.allData = deepcopy(mDict["allData"])
        obj.data = deepcopy(mDict["data"])
        obj.metadata = deepcopy(mDict["metadata"])
        obj.pythonCalFit = deepcopy(mDict["pythonCalFit"])
        obj.foldometerCalFit = deepcopy(mDict["foldometerCalFit"])
        obj.thermalCalibration = deepcopy(mDict["thermalCalibration"])
        obj.timeInterval = deepcopy(mDict["timeInterval"])
        return obj
    ...
    def set_WLC_ruler(self, contourLengthDNA=3460, persistenceLengthDNA=44.4, stretchModulusDNA=720, persistenceLengthProtein=0.75):
        self.paramsWLC = {}
        self.paramsWLC["contourLengthDNA"] = contourLengthDNA
        self.paramsWLC["persistenceLengthDNA"] = persistenceLengthDNA
        self.paramsWLC["stretchModulusDNA"] = stretchModulusDNA
        self.paramsWLC["persistenceLengthProtein"] = persistenceLengthProtein


    def save_m_as_multi_pickle_file(self, folderPickledData, name):
        filePath = folderPickledData + "/" + name
        if not os.path.isdir(filePath):
            os.mkdir(filePath)
        m_dict = {}
        m_dict["setup"] = self.setup
        m_dict["protein"] = self.protein
        m_dict["filePath"] = self.filePath
        m_dict["paramsWLC"] = self.paramsWLC
        m_dict["allRawData"] = self.allRawData
        m_dict["rawData"] = self.rawData
        m_dict["allData"] = self.allData
        m_dict["data"] = self.data
        m_dict["metadata"] = self.metadata
        m_dict["pythonCalFit"] = self.pythonCalFit
        m_dict["foldometerCalFit"] = self.foldometerCalFit
        m_dict["thermalCalibration"] = self.thermalCalibration
        m_dict["timeInterval"] = self.timeInterval

        for label in ["setup", "protein", "filePath", "paramsWLC", "allRawData", "rawData", "allData", "data", "metadata", \
                      "pythonCalFit", "foldometerCalFit", "thermalCalibration", "timeInterval"]:
            with open(filePath + "/" + label + ".pickle", 'wb') as f:
                pickle.dump(m_dict[label], f)