...
def identify_unfolding_events(dataParent, axis="x", forceChannel="force", window=15, STDthreshold=0.8, forceThreshold=5):
    """
...
def find_unfolding_events(data, axis="x", forceChannel="force", distanceChannel="surfaceSep", unfoldingThreshold=1,
                          forceThreshold=5, rollingWindow=5, unfoldingWindow=15, **kwargs):
    """
    Function to calculate the force change in unfolding events with the mean before and after the identified event
    ...
    axis = axis.upper()
    data_unfolding = identify_unfolding_events(data, axis, forceChannel, rollingWindow, unfoldingThreshold, forceThreshold)
    data["unfolding"] = data_unfolding
