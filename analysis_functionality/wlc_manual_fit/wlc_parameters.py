...
def select_parameters(thermalCalibration, paramsWlc):
    ...
    forceHomothety = float(st.sidebar.text_input("forceHomothety = 1", str(1.0)))
    ...
    foldometerCalFit["stiffness"] = thermalCalibration["stiffness"] * stiffnessModification * forceHomothety
    foldometerCalFit["alpha"] = thermalCalibration["alpha"] / (diffusionModification ** 0.5)
    foldometerCalFit["beta"] = thermalCalibration["beta"] * forceHomothety  # * (stiffnessModification / diffusionModification**0.5) = 1
    ...
    paramsWlcFit["persistenceLengthProtein"] = float(st.sidebar.text_input("persistenceLengthProtein = 0.75", str(paramsWlc["persistenceLengthProtein"])))
